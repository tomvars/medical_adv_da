...
from src.networks import SplitHeadModel, DiscriminatorCycleGANSimple, GeneratorUnet
...
class ICMSCModel(BaseModel):
    def __init__(self, cf, writer, results_folder, models_folder, tensorboard_folder,
        ...
        self.optimizer_seg = PCGrad(optim.Adam(
            itertools.chain(self.seg_model_target.parameters(), self.seg_model_source.parameters()), #\ , 
            lr=self.cf.lr
        ))
    ...
    def training_loop(self, source_dl, target_dl):
        ...
        cmsc_t_tumour_loss = dice_soft_loss(torch.sigmoid(outputs_target), torch.sigmoid(outputs_generated_source.detach()))
        cmsc_t_cochlea_loss = dice_soft_loss(torch.sigmoid(outputs_target2), torch.sigmoid(outputs_generated_source2.detach()))
