...
"""
FCOS. According to https://arxiv.org/pdf/1904.01355.pdf
"""

import src.medicaldetectiontoolkit.model_utils as mutils
import src.medicaldetectiontoolkit.exp_utils as utils
import sys
...
def refine_detections(anchors, probs, deltas, batch_ixs, cf):
    ...
    pre_nms_anchors = anchors[keep_arr[:, 0]].contiguous()
    pre_nms_deltas = deltas[keep_arr[:, 0]]
    
    keep = torch.arange(pre_nms_scores.size()[0]).long().to('cuda:0')
...
#         seg_logits = self.final_conv(self.intermediate_conv(fpn_outs[0]))
        ...
        return detections, class_logits, bb_outputs, seg_logits, fpn_outs
